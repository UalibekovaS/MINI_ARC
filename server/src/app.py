...
def store_logs():
    ...
    json_obj = dict(reversed(list(json_obj.items())))
    json_obj1 = dict(itertools.islice(json_obj.items(), 1))
    json_obj2 = dict(itertools.islice(json_obj.items(), 2))
    json_obj3 = dict(itertools.islice(json_obj.items(), 3))
    try:
        con = db.get_db()
        con.execute(
            "INSERT INTO logs (task_id, user_id, action_sequence) VALUES (?, ?, ?)", (json_obj3.get(
                'task'), json_obj2.get('user_id'), json.dumps((json_obj1)))
        )
        con.commit()
    except Exception as e:
        print(e)
...
def store_final_set():
    ...
    try:
        con = db.get_db()
        con.execute(
            "INSERT INTO testsets (user_id, test_id, testjson, approve, ratings, Description) VALUES (?, ?, ?, ?, ?, ?)", (json_obj.get(
                'user_id'), json_obj.get('test_id'), json.dumps(json_obj), False, 0, json_obj.get('description'))
        )
        con.commit()
    except Exception as e:
        print(e)
...
def update_approval():
    ...
    try:
        con = db.get_db()
        _query = "UPDATE testsets SET approve=" + \
            str(json_obj.get('approve')) + " WHERE test_id='" + \
            json_obj.get('test_id') + "'"
        print(_query)
        con.execute(_query)
        con.commit()
    except Exception as e:
...
def delete_set():
    ...
    try:
        con = db.get_db()
        _query = "DELETE FROM testsets WHERE test_id='" + \
            json_obj.get('test_id') + "'"
        print(_query)
        con.execute(_query)
        con.commit()
...
def search_test():
    ...
    if (_user_id):
        if (_description):
            query_ = "SELECT * from testsets WHERE user_id='" + \
                _user_id + "' AND description='"+_description+"'"
        else:
...
def save_json_test():
    ...
    for d in _list:
        query_ = "SELECT * from testsets WHERE test_id='" + \
            d['testid'] + "' AND user_id='" + d['userid'] + "' AND approve=1"
        try:
            cur = db.get_db().cursor()
            cur.execute(query_)
            data = [dict((cur.description[i][0], value)
...
def save_json_logs():
    try:
        cur = db.get_db().cursor()
        cur.execute("SELECT user_id FROM logs ORDER BY id DESC LIMIT 1")
        userid = cur.fetchone()[0]
        cur = db.get_db().cursor()
        cur.execute("SELECT task_id FROM logs ORDER BY id DESC LIMIT 1")
        ...
        final_set = cur.fetchone()
        final_set = dict(final_set)
        with open('../../data/MiniARC_traces/{}_{}_{}.json'.format(rowid, taskid, userid), 'w', encoding='utf-8') as f:
