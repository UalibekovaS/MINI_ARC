// Internal state.
var log_output_grid = new Grid(5, 5);
var log_input_grid = new Grid(5, 5);
var CURRENT_INPUT_GRID = new Grid(5, 5);
var CURRENT_OUTPUT_GRID = new Grid(5, 5);
var testinputpairs = new Array();
var testoutputpairs = new Array();
...
function resetTask() {
    testinputpairs.length = 0;
    testoutputpairs.length = 0;
    prevTime = new Date()
    tries = 0;
    logSaved = false;
...
//Function for saving the current background color during the rotation
function syncFromDataGridToEditionGridRotate(pad) {
    refreshEditionGrid($('#output_grid .edition_grid'), CURRENT_OUTPUT_GRID, pad);
}
...
function loadJSONTask(train, test) {
    ...
    for (var i = 0; i < train.length; i++) {   
        ...
        testinputpairs.push(input_grid);
        testoutputpairs.push(output_grid);
    }
    ...
    log_input_grid = JSON.stringify(CURRENT_INPUT_GRID);
...
function submitSolution() {
    testinputpairs = JSON.stringify(testinputpairs);
    testoutputpairs = JSON.stringify(testoutputpairs);
    sessionStorage.setItem('testinputpairs', testinputpairs);
    localStorage.setItem('testinputpairs', testinputpairs);
    sessionStorage.setItem('testoutputpairs', testoutputpairs);
    localStorage.setItem('testoutputpairs', testoutputpairs);
    sessionStorage.setItem('task_name_test', task_name_test);
    localStorage.setItem('task_name_test', task_name_test);
    sessionStorage.setItem('log_input_grid', log_input_grid);
    localStorage.setItem('log_input_grid', log_input_grid);
    syncFromEditionGridToDataGrid();
    ...
    $.ajax({
        url:saveLogs(logs, true),
        success:function(){
        saveLogAsFile(logs, true);
     }
     })

    reference_output = convertSerializedGridToGridObject(reference_output);
    reference_output = JSON.stringify(reference_output)
    log_output_grid = reference_output;
    sessionStorage.setItem('log_output_grid', log_output_grid);
    localStorage.setItem('log_output_grid', log_output_grid);
        ...
        $('.selectable_grid').selectable(
            {
                ...
                selected: function (event, ui) {
                    ...
                    $('.ui-selected').each(function (i, selected) {
                        row = $(selected).attr('x');
                        col = $(selected).attr('y');
                        LAYERS[currentLayerIndex].addCell(new Cell($(selected).attr('x'), $(selected).attr('y'), $(selected).attr('symbol'), true));
                        console.log(LAYERS[currentLayerIndex].cells)
                        row = $(selected).attr('x');
                        col = $(selected).attr('y');
                        LAYERS[currentLayerIndex].cells.forEach(function (cell) {
...
function rotateCells() {
    ...
    $('.edition_grid').find('.ui-selected').each(function (i, cell) {
        ...
        for (var j = 0; j < currCells.length; j++) {
            if ((currCells[j].row == row || currCells[j].col == col || currCells[j].row != row ||currCells[j].col != col) && currCells[j].selected == true) {
                if(!ind.includes(j)){
                    ind.push(j);
                }
            }
...
function saveLogAsFile(log_list, correct) {
    $.ajax({
        type: 'POST',
        url: '/save',
        data: JSON.stringify(log_list),
        dataType: 'json',
        asyn:false,
        contentType: 'application/json; charset=utf-8'
    }).done(function(msg) {
